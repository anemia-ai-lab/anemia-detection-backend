...
| Docker prod | El `Dockerfile` instala `libgl1-mesa-glx`, OpenCV headless antes de MediaPipe y descarga `hand_landmarker.task` en build. Tras cambios en Docker, **rebuild + redeploy ECS** antes de smoke prod. |

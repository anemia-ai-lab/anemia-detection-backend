...
- **Calibración final:** `artifacts/runs/calibration_ensemble_ghana_v2.*` (Platt unique-hash).
...
Ghana: prefijos `Anemic-FN-*` → `positive`, `Non-Anrmic-FN-*` → `negative` (typos del autor). Por defecto original-only (~507 sujetos). Con `--include-augmented` se aplica dedup por SHA-256 de contenido. En macOS, si TensorFlow aborta al importar, el preparador usa `sips` para redimensionar.
